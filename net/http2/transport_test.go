...
func tlsclientConnPing(t *testing.T) {
	st := newServerTester(t, func(w http.ResponseWriter, r *http.Request) {}, optOnlyServer)
	defer st.Close()
	tr := &Transport{TLSClientConfig: tlsConfigInsecure}
	defer tr.CloseIdleConnections()
	cc, err := tr.dialClientConn(st.ts.Listener.Addr().String(), st.ts.Listener.Addr().String(), false)
	if err != nil {
		t.Fatal(err)
	}
...
func tlsclientConnClose(t *testing.T, closeMode closeMode) {
	clientDone := make(chan struct{})
	defer close(clientDone)
...
func tlsclientConnCloseAtHeaders(t *testing.T) {
	tlsclientConnClose(t, closeAtHeaders)
}
...
func tlsclientConnCloseAtBody(t *testing.T) {
	tlsclientConnClose(t, closeAtBody)
}
...
func tlsclientConnShutdown(t *testing.T) {
	tlsclientConnClose(t, shutdown)
}
...
func tlsclientConnShutdownCancel(t *testing.T) {
	tlsclientConnClose(t, shutdownCancel)
}
...
func tlsclientConnTooIdle(t *testing.T) {
	tests := []struct {
		cc   func() *ClientConn
		want bool
	}{
