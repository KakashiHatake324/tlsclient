...
func (t *Transport) newClientConn(c net.Conn, singleUse bool) (*ClientConn, error) {
	...
	cc.fr.ReadMetaHeaders = hpack.NewDecoder(65536, nil)
	...
	for len(hdrs) > 0 && cc.werr == nil {
		...
		if first {
			cc.fr.WriteHeaders(HeadersFrameParam{
				...
				// setting priority weight as 220
	...
	enumerateHeaders := func(f func(name, value string)) {
		...
		for k, vv := range req.Header {
			if strings.EqualFold(k, "host") || strings.EqualFold(k, "content-length") {
				...
				if len(orderedKeys) != 0 && strings.Contains(strings.ToLower(req.Header.Get("Header-Order")), "cookie") {
					// do nothing
				} else {
					for _, v := range vv {
						for {
			...
			// If length of orderedKeys is 0 then
		...
		// If Length of orderedKeys not 0 then
		// Headers will be sent in defined order.
		if len(orderedKeys) != 0 {
			for i := 0; i < len(orderedKeys); i++ {

				if orderedKeys[i] == "cookie" {
					arr := strings.Split(orderedValues[i], ";")
					...
					f(orderedKeys[i], orderedValues[i])
				}

			}
		}

		// If content-length not defined by user then add it.
		if shouldSendReqContentLength(req.Method, contentLength) && req.Header.Get("content-length") == "" {
